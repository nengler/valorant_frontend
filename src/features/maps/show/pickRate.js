...
export default function PickRate({ agentPercentages }) {
  ...
  const [agentStatistics, setAgentStatistics] = useState(agentPercentages);

  useEffect(() => {
    if (agentPercentages) {
      setAgentStatistics(agentPercentages);
    }
  }, [agentPercentages]);
  ...
  return (
    <Box mb="20" ref={(ref) => setBarGraphContainer(ref)}>
      <Heading mb="2" as="h4" size="md">
        Agent Pickrate
      </Heading>
      <Box h={agentStatistics.length * 52}>
        {agentStatistics.map((agentStatistic, index) => (
          <AgentBarChart
            key={agentStatistic.name}
            containerWidth={barGraphContainer?.offsetWidth}
            agent={agentStatistic}
            agentImage={agentObject[agentStatistic.name]}
            index={index}
          />
        ))}
      </Box>
      <Box pos="relative" ml="56px" borderTop="3px solid black">
